...
class Multi_critical_filtration
{
 public:
   ...
   */
  using Single_point = One_critical_filtration<T>;
  // TODO: not the best name...
  using Points = std::vector<Single_point>;               /**< Container type for the filtration values. */
  using iterator = typename Points::iterator;             /**< Iterator type for the point container. */
  using const_iterator = typename Points::const_iterator; /**< Const iterator type for the point container. */
  ...
  friend bool operator<(const Multi_critical_filtration &a, const Multi_critical_filtration &b)
  {
    for (std::size_t i = 0u; i < b.multi_filtration_.size(); ++i) {
      //for each point in b, verify if it is strictly in the cone of at least one point of a
   ...
   */
  void push_to_least_common_upper_bound(const Single_point &x)
  {
   ...
   */
  void pull_to_greatest_common_lower_bound(const Single_point &x)
  {
   ...
   * added "by hand" either trough the constructor or with @ref add_guaranteed_point "".
   */
  void simplify()
  {
    std::size_t end = 0;
 ...
 private:
  Points multi_filtration_; /**< Container for generators. */
